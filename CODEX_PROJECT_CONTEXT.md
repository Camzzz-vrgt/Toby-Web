...
- Camzzz, Bog, ChatGPT, and Toby Fox use credit images stored in `files/credits/`; the Credits page references their raw GitHub URLs so they also render when the single-file launcher is opened locally.
...
- Frostveil was extracted from its fused `Frostveil.exe` LOVE archive and retains Frostveil's own customized Kristal 0.10 engine. Only targeted web-compatibility patches are applied: synchronous asset loading, native-library shims, guarded optional FFI blending, browser-safe tiled GID parsing, and Lua control-flow rewrites. Do not replace this engine with another Kristal game's runtime, because that mixes game-specific assets and behavior. Its roughly 112 MB package is stored as `deltarune-frostveil.love.part1` through `.part3`; `files/deltarune-frostveil/player.js` downloads and joins those parts before starting love.js. Keep all three parts together and update the split-loader count if the package is rebuilt.
...
- Rebuild Offline Bingo only when its patch or source mod changes:
