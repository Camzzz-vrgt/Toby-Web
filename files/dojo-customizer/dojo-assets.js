(function () {
  if (window.__dulDojoAssetRouterInstalled) return;
  window.__dulDojoAssetRouterInstalled = true;
  ...
  function resolveDojoAssetUrl(value) {
  ...
  window.fetch = function (resource, ...args) {
    ...
    const resolvedUrl = resolveDojoAssetUrl(requestUrl);
  ...
  XMLHttpRequest.prototype.open = function (method, url, ...rest) {
    return originalOpen.call(this, method, resolveDojoAssetUrl(url), ...rest);
  };
  ...
  if (mediaSrcDescriptor && mediaSrcDescriptor.get && mediaSrcDescriptor.set) {
    Object.defineProperty(HTMLMediaElement.prototype, "src", {
      ...
      set: function (value) {
        mediaSrcDescriptor.set.call(this, resolveDojoAssetUrl(value));
      }
    });
  }
  ...
  HTMLMediaElement.prototype.setAttribute = function (name, value) {
    if (String(name).toLowerCase() === "src") value = resolveDojoAssetUrl(value);
    return originalSetAttribute.call(this, name, value);
  };

  const NativeAudio = window.Audio;
  window.Audio = function (src) {
    return arguments.length > 0 ? new NativeAudio(resolveDojoAssetUrl(src)) : new NativeAudio();
  };
